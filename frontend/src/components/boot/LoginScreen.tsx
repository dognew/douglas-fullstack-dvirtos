...
/**
 * Layer 2: Display Manager / Login Screen
 * Responsibility: User authentication and session environment selection.
 * This component acts as the bridge between Boot stage and the Graphical Desktop.
 */
export default function LoginScreen({ onComplete, onReboot }: {
  ...
  /**
   * Handles profile selection and ensures Layer 0 (SessionManager) 
   * receives authentication before transitioning to DESKTOP stage.
   */
  const handleProfileSelect = (profileId: string) => {
    // Persistent storage for session dump/recovery
    sessionStorage.setItem('selected_visitor_type', profileId);
    
    // Register authentication in the system kernel
    setAuth(profileId);
    
    // Transition to the next boot stage
    onComplete();
  };
    ...
    >
      ...
      <div className="z-10 w-full max-w-4xl bg-black/20 backdrop-blur-md border border-white/10 p-6 md:p-12 rounded-2xl shadow-2xl flex flex-col items-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 w-full mb-10 md:mb-16 bg-transparent">
          {profiles.map((profile) => (
            <button
              ...
              className="flex flex-col items-center group transition-all duration-300 bg-transparent border-none p-0 outline-none cursor-pointer"
            >
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-white/10 bg-transparent">
          <button
            onClick={onReboot}
            className="text-xs text-white/40 hover:text-[#B87C00] transition-colors flex items-center gap-2 font-light bg-transparent border-none outline-none cursor-pointer"
          >
            <i className="bi bi-arrow-clockwise"></i> Reiniciar sistema
          </button>
          
          <div className="text-[#E4C844]/60 text-xs font-mono bg-transparent">
            {/* Reverted BootTimer to original usage without disallowed className prop */}
            <BootTimer
              seconds={30}
